...
class TradingService:
    def __init__(self, db: Session, user_id: int,
                 price_provider: Callable[[str], Optional[float]] = get_current_price):
        self.db = db
        self.user_id = user_id
        self.price_provider = price_provider

    def _resolve_market_price(self, stock_code: str) -> Optional[float]:
        price = self.price_provider(stock_code)
    ...
    def buy_stock(self, stock_code: str, shares: int, client_price: Optional[float] = None,
        ...
        market_price = self._resolve_market_price(stock_code)
        if market_price is None:
            return False, f"No market quote available for {stock_code}"

        stock = self._get_or_create_stock(stock_code)

        effective_price = market_price + slippage_per_share
    ...
    def sell_stock(self, stock_code: str, shares: int, client_price: Optional[float] = None,
                   fee_rate: float = 0.001, min_fee: float = 1.0,
                   slippage_per_share: float = 0.01) -> Tuple[bool, str]:
        user = self._get_user()
        if not user:
        ...
        effective_price = max(0.01, market_price - slippage_per_share)
        total_amount = shares * effective_price
        fee = max(total_amount * fee_rate, min_fee)
    ...
    def get_portfolio(self) -> list:
        ...
        for stock_id, shares, avg_cost in summaries:
            ...
            if not stock:
                continue
            current_price = self._resolve_market_price(stock.code)
