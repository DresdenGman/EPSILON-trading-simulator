...
2. Click `Run ε experiment` once. The page makes two real backtest requests.
...
5. Click `Run pre-specified replication`. The page makes two more real backtest requests.
...
The Python tests cover weekday-only history, OHLC consistency, temporal continuity, exact repeatability, fixed non-overlapping replication windows, and different `PYTHONHASHSEED` processes. The web tests cover local diagnosis and cross-window replication cases.

## Reproducibility contract

Same commit/worktree, experiment protocol, CSP-v1 model version, fixed windows, universe, capital, fee assumptions, slippage values, and strategy configuration should produce the same controlled experiment outputs across restarts and processes.
