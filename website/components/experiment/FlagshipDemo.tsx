...
export default function FlagshipDemo() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section id="demo-step-1" className="surface-card rounded-box border-primary/20 px-5 py-7 md:px-8 md:py-10">
        ...
        <div className="mt-8 border-y border-base-300/70 py-5">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-[1fr_auto_1fr] md:items-center">
            <div className="rounded-box border border-base-300/70 px-4 py-3">
            ...
            <div className="rounded-box border border-primary/35 bg-primary/5 px-4 py-3">
        ...
        <div className="mt-5 rounded-box border border-warning/25 bg-warning/5 px-4 py-4">
        ...
        <div className="mt-6 flex flex-wrap items-end justify-between gap-5">
          ...
          <button onClick={runExperiment} disabled={state === "running"} className="btn btn-primary btn-sm min-w-44 font-mono text-2xs">
            {state === "running" ? "Running…" : state === "complete" || state === "inconclusive" ? "Run primary again" : "Run primary ε test"}
          </button>
        </div>
      </section>

      <section id="demo-step-2" className="surface-card rounded-box border-base-300/80 px-5 py-6 md:px-8">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          ...
          <div className="rounded-box border border-primary/25 bg-primary/5 p-4 font-mono text-xs"><div className="text-primary/70">+ε · $0.020/share</div><div className="mt-2 text-base-content/55">{perturbed ? "COMPLETE" : state === "running" ? "RUNNING…" : "WAITING"}</div></div>
        </div>

        {state === "failed" && <div className="mt-4 rounded-box border border-error/30 bg-error/5 p-4 text-sm text-error"><div className="font-mono text-2xs uppercase tracking-[0.14em]">Engine unavailable</div><p className="mt-2 text-error/80">{error} No primary conclusion generated.</p><button onClick={runExperiment} className="btn btn-outline btn-error btn-xs mt-3 font-mono text-2xs">Retry primary</button></div>}
      </section>

      {baseline && perturbed && <section id="demo-step-3" className="surface-card rounded-box border-base-300/80 px-5 py-6 md:px-8"><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-mono text-2xs uppercase tracking-[0.18em] text-primary">03 / Diagnosis</div><span className="font-mono text-2xs text-base-content/45">{DEMO_EXPERIMENT.startDate} → {DEMO_EXPERIMENT.endDate}</span></div><div className="mt-5 grid grid-cols-2 gap-5 md:grid-cols-4"><Metric label="Baseline total return" value={formatReturn(baseline.totalReturn)} muted /><Metric label="+ε total return" value={formatReturn(perturbed.totalReturn)} muted /><Metric label="Δ return" value={`${delta !== null && delta >= 0 ? "+" : ""}${delta?.toFixed(2)} pp`} muted /><Metric label="Executed trades" value={`${baseline.tradeCount} → ${perturbed.tradeCount}`} muted /></div><div className="mt-5 border-t border-base-300/70 pt-3 font-mono text-2xs uppercase tracking-[0.14em] text-primary/70">Decision input: return sign · One variable changed: execution friction</div></section>}

      {diagnosis && <section id="demo-step-4" className={`surface-card rounded-box border px-5 py-6 md:px-8 ${diagnosis.outcome === "reversed" ? "border-warning/50 bg-warning/5" : "border-primary/25"}`}><div className="flex flex-wrap items-center justify-between gap-3"><div className="font-mono text-2xs uppercase tracking-[0.18em] text-primary">04 / Primary conclusion</div><span className={`badge badge-outline font-mono text-2xs ${diagnosis.outcome === "reversed" ? "border-warning/50 text-warning" : "border-primary/50 text-primary"}`}>{diagnosis.outcome === "preserved" ? "PRESERVED" : "REVERSED"}</span></div><h2 className="mt-4 text-xl font-semibold">{conclusionText}</h2><p className="mt-2 text-sm leading-relaxed text-base-content/60">{diagnosis.effect === "weakened" ? "Execution friction weakened the observed result without reversing its direction." : "The perturbation did not weaken the observed result."}</p></section>}
      ...
      <section id="demo-step-6" className="surface-card rounded-box border-warning/25 bg-warning/5 px-5 py-6 md:px-8"><div className="font-mono text-2xs uppercase tracking-[0.18em] text-warning/80">06 / Limits</div><h2 className="mt-3 text-xl font-semibold">A local sensitivity result is not general robustness.</h2><div className="mt-4 grid gap-2 font-mono text-2xs uppercase tracking-[0.12em] text-base-content/50 md:grid-cols-4"><span>1 strategy configuration</span><span>2 controlled windows</span><span>1 perturbed parameter</span><span>1 ε magnitude</span></div><p className="mt-5 max-w-3xl text-sm leading-relaxed text-base-content/65">The experiment shows whether this specific conclusion survived this specific perturbation and a pre-specified replication window. It does not establish historical market validity, profitability, statistical significance, general robustness, or predictive performance.</p></section>
    </div>
  );
}
